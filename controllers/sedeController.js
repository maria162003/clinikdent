...
exports.obtenerSedes = async (req, res) => {
  ...
  try {
    ...
    const [sedes] = await db.query(query);
...
exports.crearSede = async (req, res) => {
  ...
  try {
    const insertQuery = `
      INSERT INTO sedes (nombre, direccion, telefono, ciudad, fecha_creacion, estado)
      VALUES (?, ?, ?, ?, NOW(), 'activa')
    `;
    
    const [result] = await db.query(insertQuery, [nombre, direccion, telefono || null, ciudad]);
    console.log('✅ Sede creada con ID:', result.insertId);
...
exports.eliminarSede = async (req, res) => {
  ...
  try {
    ...
    const [inventario] = await db.query('SELECT COUNT(*) as total FROM inventario_equipos WHERE sede_id = ?', [id]);
    ...
    const deleteQuery = 'DELETE FROM sedes WHERE id = ?';
...
exports.obtenerSedePorId = async (req, res) => {
  ...
  try {
    const query = 'SELECT * FROM sedes WHERE id = ?';
