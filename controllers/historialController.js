...
exports.registrarHistorial = async (req, res) => {
  ...
  try {
    await db.query('INSERT INTO historial_clinico (paciente_id, odontologo_id, diagnostico, tratamiento_resumido, fecha, archivo_adjuntos) VALUES (?, ?, ?, ?, ?, ?)', [paciente_id, odontologo_id, diagnostico, tratamiento_resumido, fecha, archivo_adjuntos]);
    res.json({ msg: 'Historial registrado.' });
  } catch (err) {
    res.status(500).json({ msg: 'Error al registrar historial.' });
  }
};
...
exports.actualizarHistorial = async (req, res) => {
  ...
  try {
    const [result] = await db.query(`
      ...
      SET diagnostico = ?, tratamiento_resumido = ?, fecha = ?, archivo_adjuntos = ?
      WHERE id = ?
