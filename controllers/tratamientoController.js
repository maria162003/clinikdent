...
exports.crearTratamiento = async (req, res) => {
  ...
  try {
    await db.query('INSERT INTO tratamientos (nombre, descripcion, costo_estimado) VALUES (?, ?, ?)', [nombre, descripcion, costo_estimado]);
...
exports.asignarTratamientoAPaciente = async (req, res) => {
  ...
  try {
    const [result] = await db.query(
      'INSERT INTO paciente_tratamientos (paciente_id, tratamiento_id, odontologo_id, fecha_inicio, fecha_fin_estimada, costo_estimado, estado, descripcion, observaciones) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', 
    ...
    res.json({ success: true, message: 'Tratamiento asignado al paciente.', id: result.insertId });
  } catch (err) {
...
exports.eliminarTratamientoPaciente = async (req, res) => {
  ...
  try {
    await db.query('DELETE FROM paciente_tratamientos WHERE id = ?', [id]);
